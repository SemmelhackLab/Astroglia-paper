...
DROPBOX_DEMO_FOLDER_URL = "https://www.dropbox.com/scl/fi/7wkruxep2qujr1va386va/demo_data_F1_20250807-F2.zip?rlkey=z9gln3p8cose6qepwx39zh5kr&st=bwl8h4r8&dl=0"


def _dropbox_force_dl1(url: str) -> str:
...
def _discover_dropbox_zip_url(folder_url: str) -> str:
    ...
    matches = re.findall(r"https://www\.dropbox\.com/[^\"]+?\.zip\?[^\"]+", html)
    if not matches:
        raise RuntimeError("Could not discover a .zip download link from the provided Dropbox folder URL.")
    return matches[0]


def download_dropbox_demo_data(project_dir: Path, folder_url: str = DROPBOX_DEMO_FOLDER_URL) -> Path:
    project_dir = Path(project_dir)
    zip_url = _discover_dropbox_zip_url(folder_url)
    zip_url = _dropbox_force_dl1(zip_url)

    zip_path = project_dir / "demo_data_dropbox.zip"
    ...
    if not zip_path.exists() or zip_path.stat().st_size <= 0:
        raise RuntimeError("Downloaded demo_data_dropbox.zip is missing or empty.")
...
def test_dropbox_demo_data_download(folder_url: str = DROPBOX_DEMO_FOLDER_URL) -> bool:
    zip_url = _discover_dropbox_zip_url(folder_url)
    zip_url = _dropbox_force_dl1(zip_url)
    req = urllib.request.Request(
        zip_url,
